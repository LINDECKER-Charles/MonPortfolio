...
- **`_deploy.yml`** : **un** job SSH paramétré, identique staging/prod : pousse le `.env`, `git reset`
  sur la branche suivie, converge l'edge (`/opt/edge`), `compose pull` (retry) + `up -d`, puis
  **vérifie** : `/health` dans le conteneur, TLS + `/health` + marqueur `ng-server-context="ssg"`
  sur chaque domaine de `CADDY_DOMAINS` (self-heal du certificat edge si besoin).
...
```
/opt/edge/                      # edge proxy partagé (caddy-docker-proxy + socket proxy RO)
├── compose.yaml, Caddyfile     # copiés depuis infra/edge à chaque déploiement
└── .env                        # ACME_EMAIL (secret GitHub)
...
`COMPOSE_PROJECT_NAME`, TLS centralisé. `infra/edge` doit rester **identique** entre repos
partageant un hôte (chaque pipeline le réécrit).

## Migration vers le nouveau serveur (one-shot)

1. **Serveur** : Docker Engine + `docker compose`, `git`, `curl` ; ports 80/443 ouverts.
   Si un autre projet (LoDB) y a déjà posé l'edge, rien à faire côté edge.
...
| Edge | `docker compose -f /opt/edge/compose.yaml logs -f caddy` (émission certs) |
